# app_streamlit.py — full app that meets the outlined requirements
...
with tab_upload:
    ...
    with col2:
        default_hint = ss.data_path or "archive/events.csv (example) OR a .parquet path"
        ...
        try:
            ...
            if st.button("Train model", type="primary"):
                ...
                if model_type == "RandomForestClassifier":
                    ...
                    clf = RandomForestClassifier(featuresCol="features", labelCol="__label",
                                                 numTrees=200, maxDepth=20, maxBins=128, seed=42)
                else:
                    from pyspark.ml.classification import LogisticRegression
                    clf = LogisticRegression(featuresCol="features", labelCol="__label")
                stages += [clf]
                ...
                st.success(f"Trained {model_type}. AUROC={aucroc:.3f}, AUPRC={aucpr:.3f}")
                ss.model = model
...
with tab_predict:
    ...
    if not ss.model:
        ...
        # Make p1 probability if classification
        if "probability" in preds.columns:
            preds = preds.withColumn("prob_arr", vector_to_array("probability")).withColumn("p1", F.col("prob_arr")[1]).drop("prob_arr")

        # Show a small preview
        st.write("Scored preview")
        st.dataframe(preds.limit(50).toPandas(), use_container_width=True)

        # Simple viz: histogram of p1 (if present)
        if "p1" in preds.columns:
            pdf_hist = preds.select("p1").sample(False, 0.05, seed=42).toPandas()
            st.bar_chart(np.histogram(pdf_hist["p1"], bins=20, range=(0,1))[0])
        ...
        if set(["session_id","itemid"]).issubset(set(preds.columns)) and "p1" in preds.columns:
            from pyspark.sql.window import Window
            w = Window.partitionBy("session_id").orderBy(F.col("p1").desc())
            topk = preds.select("session_id","itemid", F.col("p1").alias("p_buy")) \
                        .withColumn("rk", F.row_number().over(w)).filter("rk<=@k").drop("rk")
            st.write("Top-K preview")
            st.dataframe(topk.limit(100).toPandas(), use_container_width=True)

            # Downloads
            topk_pdf = topk.toPandas()
            download_button_df(topk_pdf, "Download Top-K CSV", "topk.csv")

        # Full predictions download
        preds_pdf = preds.limit(100000).toPandas()  # guard memory
